...
$botEnvFile = Join-Path $projectDir "config\.env.school.bot"
$botEnvExample = Join-Path $projectDir "config\.env.school.bot.example"
...
if (-not (Test-Path -LiteralPath $botEnvFile)) {
    Copy-Item -LiteralPath $botEnvExample -Destination $botEnvFile
    Write-Host "已创建 config\.env.school.bot，请填入独立学校机器人的 BotID 和 Secret。"
}
